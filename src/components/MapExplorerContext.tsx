...
export function MapExplorerContextProvider(props: MapExplorerContextProviderProps) {
    ...
    const setCamera = useCallback((cameraOrSetter: Camera | ((oldCamera: Camera) => Camera)) => {
    ...
    const loadImage = useCallback((src: string) => {
        const canvas = canvasRef.current;
        if (canvas === null) {
            throw new Error(`"loadImage()" can only be called after canvas initialization`);
        }
...
export function useMapExplorer() {
    ...
    if (ctx === null) {
        throw new Error(`"useMapExplorer()" must only be used inside a "MapExplorerContextProvider"`);
    }
    return ctx;
}
